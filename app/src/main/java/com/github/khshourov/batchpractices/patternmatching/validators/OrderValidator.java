package com.github.khshourov.batchpractices.patternmatching.validators;
...
public class OrderValidator implements Validator {
  ...
  protected void validateLineItems(List<LineItem> lineItems, Errors errors) {
    ...
    for (LineItem lineItem : lineItems) {
      ...
      if ((BD_MIN.compareTo(lineItem.getPrice()) > 0)
          || (BD_MAX.compareTo(lineItem.getPrice()) < 0)) {
        prices = false;
      }
      ...
      BigDecimal coef =
          BD_100.subtract(lineItem.getDiscountPerc()).divide(BD_100, 4, RoundingMode.HALF_UP);
      ...
      BigDecimal discountedPrice =
          lineItem.getPrice().multiply(coef).subtract(lineItem.getDiscountAmount());

      // price for single item = discountedPrice + shipping + handling
      BigDecimal singleItemPrice =
          discountedPrice.add(lineItem.getShippingPrice()).add(lineItem.getHandlingPrice());
  ...
  protected void validateShipping(ShippingInfo shipping, Errors errors) {
    ...
    if (StringUtils.hasText(shipping.getShippingInfo())) {
      validateStringLength(
          ...
          "error.shipping.shippinginfo.length",
          100);
    }
  }
  ...
  protected void validateAddress(Address address, Errors errors, String prefix) {
    if (address != null) {
      if (StringUtils.hasText(address.getAddressee())) {
        validateStringLength(
            address.getAddressee(),
            errors,
            prefix + ".addressee",
            "error.baddress.addresse.length",
            60);
      }
      ...
      validateStringLength(
          ...
          errors,
          prefix + ".addrLine1",
          "error.baddress.addrline1.length",
          50);
      ...
      if (StringUtils.hasText(address.getAddrLine2())) {
        validateStringLength(
            ...
            errors,
            prefix + ".addrLine2",
            "error.baddress.addrline2.length",
            50);
      }
      validateStringLength(
          address.getCity(), errors, prefix + ".city", "error.baddress.city.length", 30);
      validateStringLength(
          address.getZipCode(), errors, prefix + ".zipCode", "error.baddress.zipcode.length", 5);
      ...
      if ((!StringUtils.hasText(address.getState())
              && ("United States".equals(address.getCountry()))
      ...
      validateStringLength(
          address.getCountry(), errors, prefix + ".country", "error.baddress.country.length", 50);
    }
  }

  protected void validateStringLength(
      String string, Errors errors, String field, String message, int length) {
